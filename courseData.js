...
function handleAIRequest() {
  ...
  try {
  if (!_activeLesson) {
    if (output) output.innerHTML = '<span style="color:#fbbf24;">Please start a lesson first.</span>';
    return;
  }
  ...
  .catch(function(e){
    output.innerHTML = '<span style="color:var(--red);">Error contacting AI — check your connection and try again.</span>';
    console.error('handleAIRequest fetch error:', e);
  });
  } catch(syncErr) {
    if (output) output.innerHTML = '<span style="color:var(--red);">Unexpected error — please refresh and try again.</span>';
    console.error('handleAIRequest sync error:', syncErr);
  }
}
...
function updateProgressBars() {
  ...
  FINCY_COURSE.stages.forEach(function(stage, si) {
    ...
    if (sBtn) {
      var stage = FINCY_COURSE.stages[si];
      if (stage && stage.isCertification) {
        // Cert stage: check certification unlock condition
        var certUnlocked = typeof isCertificationUnlocked === 'function' && isCertificationUnlocked();
        var p = loadProgress();
        sBtn.classList.toggle('lh-locked', !certUnlocked);
        sBtn.textContent = p.certified ? '🏆 View Certificate' : (certUnlocked ? 'Begin Assessment →' : '🔒 Complete Stages 1–4 First');
        var prog = document.getElementById('lh-stage-prog-' + si);
        if (prog) prog.textContent = p.certified ? 'Certified ✓ — ' + (p.certScore || 0) + '%' : (certUnlocked ? 'Ready to assess' : 'Complete stages 1–4 first');
      } else if (!isStageUnlocked(si)) {
...
function initLearningHub() {
  ...
  /* FIX 03: Drain queued clicks that arrived before courseData.js loaded */
  (function() {
    var q = window._FLH_QUEUE || [];
    if (q.length) {
      q.forEach(function(call) {
        var fn = call[0];
        if (window.FincyLH && typeof window.FincyLH[fn] === 'function') {
          try { window.FincyLH[fn].apply(window.FincyLH, call.slice(1)); }
          catch(e) { console.warn('FincyLH replay:', e); }
        }
      });
      window._FLH_QUEUE = [];
    }
...
/* Queue drain moved into initLearningHub — see below */
